(function () {
  ...
  var REVIEW_SLUG = "no-verification-casino";
  var REVIEW_ID = "mu46v4zdef1uyy";
  var ozTsWidget = null;
  ...
  function applyScore(avg, count) {
    ...
    var starEl = document.getElementById("nc-oz-stars");
    var scoreEl = document.getElementById("nc-oz-score");
    var scoreN = document.getElementById("nc-oz-score-n");
    if (starEl) {
      starEl.setAttribute(
        "aria-label",
        score + " из 5" + (n > 0 ? ", по оценкам в комментариях" : "")
      );
      var span = starEl.querySelector("[aria-hidden]");
      if (span) span.textContent = stars(rounded);
    }
    if (scoreEl) {
      var meta = scoreEl.querySelector('meta[itemprop="ratingValue"]');
      if (meta) meta.setAttribute("content", score);
      while (scoreEl.firstChild) {
        if (
          scoreEl.firstChild.nodeType === 1 &&
          scoreEl.firstChild.tagName === "META"
        ) {
          break;
        }
        scoreEl.removeChild(scoreEl.firstChild);
      }
      var metas = scoreEl.querySelectorAll("meta");
      scoreEl.textContent = "";
      metas.forEach(function (m) {
        scoreEl.appendChild(m);
      });
      scoreEl.appendChild(
        document.createTextNode(score + " / 5" + (n > 0 ? " · " + n : ""))
      );
    }
    if (scoreN) scoreN.textContent = score;
  }
